...
if($dopost=='saveedit')
{
	...
	if($question=='0'){
		...
		if($answer==''){
			ShowMsg("请填写安全问题答案！","sys_admin_user_edit.php?id={$id}&dopost=edit");
			exit();
		}else{
			$answer = md5($answer);
		}
	...
	if($id!=1)
	{
		$query = "Update `#@__admin` set uname='$uname',usertype='$usertype',tname='$tname',email='$email',typeid='$typeid', question='$question', answer='$answer' $pwd where id='$id'";
	}
	else
	{
		$query = "Update `#@__admin` set uname='$uname',tname='$tname',email='$email',typeid='$typeid',question='$question', answer='$answer' $pwd where id='$id'";
	}
