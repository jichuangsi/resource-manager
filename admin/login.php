...
if($dopost=='login')
{
	...
	if(($validate=='' || $validate != $svali) && preg_match("/6/",$safe_gdopen)){
		ResetVdValue();
		$loginip = GetIP();
		$dsql->ExecuteNoneQuery("insert into `#@__admin_login_log` (userid, pwd, question, answer, errortype, ip) values ('$userid', '$pwd', '$question', '$answer', '-9', '$loginip')");
		ShowMsg('验证码不正确!','');
	} else {
		...
		if(!empty($userid) && !empty($pwd))
		{
			...
			$res = $cuserLogin->checkUser($userid, $pwd, $question, $answer);
			//success
