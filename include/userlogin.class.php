...
function ClearMyAddon($aid=0, $title='')
{
	...
	//把附件与文档关连
	//只关联最后一个文档 by caozhiyang 2010-07-03
	if(!empty($aid))
	{
		include($cacheFile);
		...
		*/
		...
		$num = sizeof($myaddons)-1;
		if(!empty($title)) {
			$dsql->ExecuteNoneQuery("Update `#@__uploads` set arcid='$aid',title='$title' where aid='{$myaddons[$num][0]}'");
		}
		else {
			$dsql->ExecuteNoneQuery("Update `#@__uploads` set arcid='$aid' where aid='{$myaddons[$num][0]}' ");
		}
...
class userLogin
{
	...
	//检验用户是否正确
	//增加安全问题答案的校验 by caozhiyang 2010-10-09
	function checkUser($username,$userpwd, $question, $answer)
	{
		global $dsql;
		$loginip = GetIP();
		...
		if(!isset($row->pwd))
		{
			$dsql->ExecuteNoneQuery("insert into `#@__admin_login_log` (userid, pwd, question, answer, errortype, ip) values ('$username', '$userpwd', '$question', '$answer', '-1', '$loginip')");
			return -1;
		}
		else if($pwd!=$row->pwd)
		{
			$dsql->ExecuteNoneQuery("insert into `#@__admin_login_log` (userid, pwd, question, answer, errortype, ip) values ('$username', '$userpwd', '$question', '$answer', '-2', '$loginip')");
			return -2;
		}else if($row->question != '0' && ($question!=$row->question || md5($answer)!=$row->answer)){
			$dsql->ExecuteNoneQuery("insert into `#@__admin_login_log` (userid, pwd, question, answer, errortype, ip) values ('$username', '******', '$question', '$answer', '-3', '$loginip')");
			return -3;
		}
		else
		{
			$loginip = GetIP();
			$dsql->ExecuteNoneQuery("insert into `#@__admin_login_log` (userid, pwd, question, answer, errortype, ip) values ('$username', '******', '******', '******', '0', '$loginip')");
			$this->userID = $row->id;
...
//			$_SESSION[$this->keepAdminStyleTag] = $adminstyle;
			$_SESSION[$this->keepAdminStyleTag]=$this->$adminstyle;
			
			//PutCookie('DedeUserID', $this->userID, 3600 * 24, '/');
			//PutCookie('DedeLoginTime', time(), 3600 * 24, '/');
			PutCookie('DedeUserID', $this->userID, 3600 * 1, '/');//1小时
			PutCookie('DedeLoginTime', time(), 3600 * 1, '/');//1小时
