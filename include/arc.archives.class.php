...
class Archives
{
	...
	function ParAddTable()
	{
		...
		$querys = "SELECT * FROM #@__uploads WHERE arcid = '$arcid'";
		$uplist = $this->dsql->GetOne($querys);
		$docurl=$uplist['url'];
		$index1=strrpos($docurl,'/');
		$index2=mb_strpos($docurl,'.');
//		$pdfurl=substr($docurl,$index1+1,strrpos($docurl, '.'));
//		$pdfurl=substr($docurl,$index1+1,$index2);
		$pdfurl=mb_substr($docurl,0,$index2);
		$this->Fields['pdfurl']=substr($pdfurl,$index1+1);
